...
# ===== 설정 =====
NODE_ID = sys.argv[1] if len(sys.argv) > 1 else os.environ.get("NODE_ID", "3")
#UP_IP   = sys.argv[2] if len(sys.argv) > 2 else os.environ.get("UP_IP", "192.168.4.1")     # 노트북 방향 이웃 (필수)
#DOWN_IP = sys.argv[3] if len(sys.argv) > 3 else os.environ.get("DOWN_IP", "192.168.4.12")   # AP 방향 이웃 (헤드/AP는 비움)
UP_IP   = sys.argv[2] if len(sys.argv) > 2 else os.environ.get("UP_IP", "10.180.86.171")     # 노트북 방향 이웃 (필수)
DOWN_IP = sys.argv[3] if len(sys.argv) > 3 else os.environ.get("DOWN_IP", "10.180.86.161")   # AP 방향 이웃 (헤드/AP는 비움)
